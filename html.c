 ...
 */
void html_format(char * rows, struct geojson_info * infos, int count,
                 char ** addl_head_ptr, const char ** body_attr_ptr, char ** body_content_ptr)
{
  ...
  static const char * caption_format =
    "<caption>"
      "<span style=\"float: right; font-size: smaller; margin-left: 4px; margin-right: 4px;\">"
        "Generated by <a style=\"font-variant: small-caps;\" target=\"_blank\" href=\"https://jeffbourdier.github.io/dumprows\">DumpRows</a>"
      "</span>"
      "<span style=\"float: left; margin-right: 8px;\">%d rows</span>"
    "</caption>";
  ...
  static const char * head_format =
    ...
    "<script type=\"text/javascript\"><!--\r\n"
      "var horizontal, usableWidth, usableHeight, halfWidth, halfHeight, tableDiv, mapDiv, map, control, "
        "tileLayer, pointLayer, nonpointLayer, rows, selectedIndex = 0, selectedGeometry, backPoint, "
        "forePoint,  backLine, foreLine, backPolygon, forePolygon, extents = [[%f, %f], [%f, %f]], "
        "geoObject = { type: 'FeatureCollection', features: [ %s ] }; "
      "function init() "
      "{ tableDiv = document.getElementById('tableDiv'); "
        ...
        "for (var n = geoObject.features.length, i = 0; i < n; ++i) "
  ...
  /* The <caption> element is built the same way regardless of anything. */
  a = (char *)malloc(n = strlen(caption_format) + 16);
  text_format(a, n, caption_format, count);

  /* If there's no geometry (or no rows), we can make this quick & easy. */
  if (!infos || !count)
  {
    *addl_head_ptr = html_element("style", table_style);
    *body_attr_ptr = NULL;
    *(p = (char *)malloc(m = (k = strlen(a)) + (n = strlen(rows)) + 1)) = '\0';
    text_append(p, m, a, k);
    text_append(p, m, rows, n);
    *body_content_ptr = html_element("table", p);
    free(a); free(p);
    return;
  }
  ...
  *body_content_ptr = (char *)malloc(m += strlen(body_format) + strlen(a));
  text_format(*body_content_ptr, m, body_format, a, q);
